...
<?php
echo '<a href="/dashboard/time_clock/ticket_manager/download2/0" class="btn btn-info">Download Data (XLS)</a>&nbsp;';
?>
</div>
...
while ($row = $r->fetchRow()) {
	...
	echo "
		<td>
			<a href='/dashboard/time_clock/ticket_manager/?approve=". $row['ticket_id'] ."&uid=".$row['uID']."&difference=". $row['hoursdifference']."&description=".$row['description']."' class='btn btn-default' type='submit' name='edit' onClick='return confirm(\"Are you sure you wish to approve this ticket? This can not be undone.\");'>Approve</a>
			<a href='/dashboard/time_clock/ticket_manager/?reject=". $row['ticket_id'] ."' class='btn btn-danger' onClick='return confirm(\"Are you sure you wish to reject this ticket? This can not be undone.\");'>Reject</a>
		</td>
	</tr>";
		}else{
