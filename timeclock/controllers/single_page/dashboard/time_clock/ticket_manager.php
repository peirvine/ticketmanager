...
class ticketmanager extends DashboardPageController {
	...
	public function view() {
		...
		if (isset($_GET['reject'])){
			...
			$q = 'UPDATE timeclock_tickets SET handled = 1, accepted = 1 WHERE ticket_id = ?';
			//$q = 'DELETE FROM `timeclock_tickets` WHERE ticket_id = ?';
			$r = $this->_db->query($q, $v);
			
			header("Location: /dashboard/time_clock/ticket_manager/");
			exit;
		}
		if (isset($_GET['approve'])){
			...
			$q = 'UPDATE timeclock_tickets SET handled = 1, accepted = 2 WHERE ticket_id = ?';
			$r = $this->_db->query($q, $v);
		...
		$query = 'SELECT * FROM `timeclock_tickets` LEFT JOIN (UserSearchIndexAttributes) ON (UserSearchIndexAttributes.uID = timeclock_tickets.uID) ORDER BY timeclock_tickets.ticket_id DESC'; 
	...
	public function download2($period_id){
		$this->_db = Loader::db();			
		$query = "SELECT * FROM `timeclock_tickets` LEFT JOIN (UserSearchIndexAttributes) ON (UserSearchIndexAttributes.uID = timeclock_tickets.uID)";
