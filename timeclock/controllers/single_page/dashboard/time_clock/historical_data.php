...
class Historicaldata extends DashboardPageController {
	...
	public function edit($id){
		...
		if(!isset($_POST['edit'])){
			...
			$v = array(intval($id));
			$query = 'SELECT * FROM timeclock_clockEverything WHERE hoursID = ? '; 
			...
			if(isset($this->error) && $this->error->has()){		
				...
				$q = 'UPDATE `timeclock_clockEverything` SET ';
	...
	public function download2($period_id){
		$this->_db = Loader::db();			
		$query = "SELECT * FROM `timeclock_clockEverything` LEFT JOIN (UserSearchIndexAttributes) ON (UserSearchIndexAttributes.uID = timeclock_clockEverything.uID)";
	...
	public function backup(){
		...
		$query1 = "INSERT INTO timeclock_clockEverythingEverything (`uID`, `clockin`, `clockout`, `description`)
                   SELECT `uID`, `clockin`, `clockout`, `description`
                   FROM timeclock_clockEverything";
       
		$this->_db->query($query1);

		$query2 = "TRUNCATE timeclock_clockEverything";
	...
	public function clockout($userID){
		$this->_db = Loader::db();
		$query = "UPDATE `timeclock_clockEverything` SET clockout = NOW(), description = 'Forced clock out by admin' WHERE `clockout` IS NULL AND uID = ". $userID ."";
