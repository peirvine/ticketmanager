	...
	<tbody id='categorylist'>
		...
		while ($hours1 = $hours->fetchRow()) {
			...
			if ($in == "in"){
				...
				<td>
					<a href='/dashboard/time_clock/clock_manager/clockout/". $hours1['uID'] ."' class='btn btn-warning'>Clock Out</a>
				</td>";
			}else{
...
while ($row = $r->fetchRow()) {
	...
	echo "<tr id='category". $row['hoursID'] ."'>
		...
		<!--<td>
			<a href='/dashboard/time_clock/clock_manager/edit/". $row['hoursID'] ."' class='btn btn-default'>Edit</a>
			<a href='/dashboard/time_clock/clock_manager/?delete=". $row['hoursID'] ."' class='btn btn-danger' onClick='return confirm(\"Are you sure you wish to delete this competition? This can not be undone.\");'>Delete</a>
		</td>-->
	</tr>";
