...
class Controller extends BlockController
{
    ...
    public function view(){
		...
		if (isset($_POST['clockin'])) {
			...
			$out = "UPDATE `a_clock` SET `description` = '". str_replace($search, $replace, $_POST['description']) ."', `clockout`=NOW() WHERE `clockout` IS NULL AND `uID`='". $u->getUserID() ."'";
			$this->_db->query($out);
			$this->set('blah','clockin');
		}
		
		$asdf = "SELECT * FROM `a_clock` WHERE `uID`='". $u->getUserID() ."' AND `clockout` IS NULL";
		...
		$query4 = "SELECT * FROM `a_clock` WHERE `clockout` = NULL GROUP BY uID, hoursID";
